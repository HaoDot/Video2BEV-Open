...
class Dataloader_University(Dataset):
    def __init__(self,root,transforms,names=['satellite','street','drone','google','bev'],**kwargs):
        ...
        self.sd_negative_sample = kwargs.get('sd_negative_sample')
        # self.index_cls_nums = 2
        self.sd_neg_sample_apth = '/18141169908/hao/dataset/university/sd_neg_samples'
    ...
    def sample_from_cls_neg_sample(self,name,cls_num):
        tmp = self.dict_path[name][cls_num]
        id_name = os.path.dirname(tmp[0]).split('/')[-1]
        img_path = glob(os.path.join(self.sd_neg_sample_apth, name, id_name, '*.jpeg'))
        if len(img_path)==0:
            # return None
            print(os.path.join(self.sd_neg_sample_apth, name,id_name))
            raise Exception('ERROR')
        # 等概率取出1个样本
        img_path = np.random.choice(list(img_path),1)[0]
        img = Image.open(img_path).convert('RGB')
    ...
    def __getitem__(self, index):
        ...
        if self.sd_negative_sample:
            img = self.sample_from_cls_neg_sample("satellite", cls_nums)
            img_s_neg = self.transforms_satellite(img)
        ...
        if hasattr(self,'transforms_bev')!=False:
            ...
            if self.sd_negative_sample:
                img = self.sample_from_cls_neg_sample("bev", cls_nums)
                img_bev_neg = self.transforms_bev(img)

            return img_s, img_st, img_d, img_google, img_bev, img_s_neg, img_bev_neg, index
        else:
...
def train_collate_fn(batch):
    ...
    if len((batch[0])) == 8:
        ...
        img_s, img_st,img_d,img_google,img_bev, img_s_neg, img_bev_neg,ids = zip(*batch)
        ...
        return [torch.stack(img_s, dim=0),ids],\
               ...
               [[torch.stack(img_google,dim=0),ids], \
                [torch.stack(img_bev,dim=0),ids], \
                [torch.stack(img_s_neg, dim=0), ids], \
                [torch.stack(img_bev_neg, dim=0), ids], \
                ]
    else:
