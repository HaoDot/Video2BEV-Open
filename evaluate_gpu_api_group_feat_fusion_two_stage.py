        ...
        if 'bev' in kwargs['query_name'] or 'drone' in kwargs['query_name']:
            ...
            for frame_idx in range(len(query_feature_full_seq)):
                ...
                with torch.no_grad():
                    ...
                    frame_query_feature_full_seq = frame_query_feature_full_seq.cuda()
                    gallery_feature_top_k = gallery_feature_top_k.cuda()
                    
                    # itm_GFLOPs = calc_flops_gpu_itm(
                    #     model,
                    #     frame_query_feature_full_seq,
                    #     gallery_feature_top_k,
                    # )
                    # itm_time = measure_forward_time_itm(model, frame_query_feature_full_seq, gallery_feature_top_k,)
                    # print()
                    # print(f'ITM: {itm_GFLOPs} GFLOPs, {itm_time} s')
                    # exit()
...
def evaluate_efficient(qf_l, ql, gf, gl, query_path_group, gallery_abs_path_l, cur_result_analysis_path, **kwargs):
    """
    ...
    if not kwargs.get('first_stage_only'):
        ...
        if 'bev' in kwargs['query_name'] or 'drone' in kwargs['query_name']:
            # satellite:KV, drone or bev:Q
            query_feature_full_seq, gallery_full_seq_path_l = \
                kwargs['query_feature_full_seq'], kwargs['gallery_full_seq_path_l']
            query_feature_full_seq = torch.load(query_feature_full_seq, map_location=gf.device)

            # TODO: use individual image in the video, then average the whole scores of itm head
            # satellite; 无需分组
            gallery_feat_top_k = [gallery_full_seq_path_l[u] for u in index_topk]
            tmp_l = []
            for tmp_path in gallery_feat_top_k:
                ff = torch.load(tmp_path, map_location=query_feature_full_seq.device)
                tmp_l.append(ff)
            gallery_feature_top_k = torch.stack(tmp_l, dim=0)
            # gallery_feature_top_k = (gallery_full_seq_path_l[list(index_topk), :])
    ...
    if (gl[index][0] != ql):
        # print('estimated category:{}, right id is :{}th, query id:{}, gallery id:{}'.format(gl[index][index[0]], (np.argwhere((index == ql))[0, 0] + 1), ql, gl[index][index[0]]))
        print('estimated category:{}, right id is :{}th, query id:{}'.format(gl[index][0], (np.argwhere((index == ql))[0, 0] + 1), ql))
        Image.fromarray(np.uint8(q_zero_g)).convert('RGB').save(
            os.path.join(os.path.dirname(cur_result_analysis_path), '{}.jpg'.format(sample_reid_name)))
    else:
        pass
...
def evaluate_gallery_group_efficient(qf, ql, gf_l, gl, query_path, gallery_abs_path_group, cur_result_analysis_path, query_name,
                           gallery_name, topk_two_stage, model, **kwargs):
    ...
    if not kwargs.get('first_stage_only'):
        ...
        if ('bev' in gallery_name) or ('drone' in gallery_name):
            ...
            tmp_l = []
            for tmp_path in gallery_feat_top_k:
                ff = torch.load(tmp_path, map_location=qf.device)
                tmp_l.append(ff)
            gallery_feat_top_k = tmp_l
    ...
    if kwargs.get('first_stage_only'):
        ...
        for idx, g_id in enumerate(visualization_index):
            if idx == 0:
                continue
            sample_reid_name = sample_reid_name + '_{:04d}'.format(gl[g_id])
    if ([gl[i] for i in index][0] != ql):
        print('estimated category:{}, right id is :{}th, query id:{}, gallery id:{}'.format([gl[i] for i in index][int(index[0])], (
                np.argwhere(([gl[i] for i in index] == ql))[0, 0] + 1), ql, [gl[i] for i in index][int(index[0])]))
        Image.fromarray(np.uint8(q_zero_g)).convert('RGB').save(
            os.path.join(os.path.dirname(cur_result_analysis_path), '{}.jpg'.format(sample_reid_name)))
    else:
        pass
...
def compose_gallery_group_label(gallery_label, gallery_img_path_l):
    """
    ...
    for i in range(len(sorted(list(gallery_name_d.keys())))):
        ...
        # assert len(gallery_feature_group[-1]) == len(gallery_abs_path_group[-1])
    # return gallery_feature_group, gallery_label_group, gallery_abs_path_group
    return gallery_label_group, gallery_abs_path_group
...
def evaluate_root(query_name, query_img_path_l, gallery_name, gallery_img_path_l, output_path, topk_two_stage, model,
    ...
    if not opt.first_stage_only:
        ...
        # query_feature, query_feature_full_seq = load_all_features_ff(result['query_f_path'][0], device='cpu', name=query_name)
        query_feature = load_all_features_ff(result['query_f_path'][0], device='cpu', name=query_name)
        # gallery_feature, gallery_feature_full_seq = load_all_features_ff(result['gallery_f_path'][0], device='cpu', name=gallery_name)
        gallery_feature = load_all_features_ff(result['gallery_f_path'][0], device='cpu', name=gallery_name)
    ...
    if 'bev' in query_name:
        ...
        # 要去掉！！
        # gallery_feature_full_seq = load_all_features_ff_full_seq(result['gallery_f_path'][0], device='cpu', name=gallery_name)
        
        query_f_path = result['query_f_path'][0]
        query_full_seq_path_l = sorted(glob(os.path.join(query_f_path, "*_ff_full_seq.pt")))
        
        gallery_f_path = result['gallery_f_path'][0]
        gallery_full_seq_path_l = sorted(glob(os.path.join(gallery_f_path, "*_ff_full_seq.pt")))
        
        
        if not opt.first_stage_only:
            query_label_group, query_abs_path_group = compose_gallery_group_label(query_label, query_img_path_l)
            ...
            query_feature_full_seq_group = query_feature
        for i in tqdm(range(len(query_label_group))):
            ...
            [ap_tmp, CMC_tmp] = evaluate_efficient(qf_l=query_feature[i], ql=query_label_group[i], gf=gallery_feature,
                                              gl=gallery_label, query_path_group=query_abs_path_group[i],
                                              gallery_abs_path_l=gallery_img_path_l,
                                              cur_result_analysis_path=cur_result_analysis_path,
                                              topk_two_stage=topk_two_stage, query_name=query_name,
                                              query_feature_full_seq=query_full_seq_path_l[i],
                                              gallery_full_seq_path_l=gallery_full_seq_path_l, model=model,
                                              txt_path=txt_path,
                                              first_stage_only=opt.first_stage_only)
            # acc_all = acc_all + acc
            if CMC_tmp[0] == -1:
        ...
        f.close()
        del query_feature,gallery_feature,
        gc.collect()
        ...
        query_f_path = result['query_f_path'][0]
        query_full_seq_path_l = sorted(glob(os.path.join(query_f_path, "*_ff_full_seq.pt")))
        # query_feature_full_seq = load_all_features_ff_full_seq(result['query_f_path'][0], device='cpu', name=query_name)
        ...
        gallery_label_group, gallery_abs_path_group = compose_gallery_group_label(gallery_label, gallery_img_path_l)
        
        gallery_label = gallery_label_group
        gallery_feature_group = gallery_feature
        del gallery_feature
        gc.collect()
        ...
        for i in tqdm(range(len(query_label))):
            ...
            query_feature_full_seq = torch.load(query_full_seq_path_l[i], map_location='cpu')
            ap_tmp, CMC_tmp = evaluate_gallery_group_efficient(qf=query_feature[i], ql=query_label[i], gf_l=gallery_feature_group,
                                                     gl=gallery_label_group, query_path=query_path,
                                                     gallery_abs_path_group=gallery_abs_path_group,
                                                     cur_result_analysis_path=cur_result_analysis_path,
                                                     query_name=query_name, gallery_name=gallery_name,
                                                     topk_two_stage=topk_two_stage,
                                                     model=model, query_feature_full_seq=query_feature_full_seq,
                                                     gallery_full_seq_path_l=gallery_full_seq_path_l,
                                                     txt_path=txt_path,
                                                     first_stage_only=opt.first_stage_only)
        ...
        with open(os.path.join(output_path, 'result.txt'), 'a+') as f:
            ...
            f.writelines(['ACC:{}%\n'.format(100 * (acc_all / len(query_label)))])
        f.close()
        del gallery_feature_group, gallery_label_group, query_feature,query_feature_full_seq,
        gc.collect()
