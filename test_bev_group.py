...
from evaluate_gpu_api_group import evaluate_root
...
model_dir_name = 'model_2024-03-06-12:15:21'
...
if opt.multi:
    ...
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                             shuffle=False, num_workers=16) for x in ['gallery_satellite', 'gallery_drone','gallery_bev','gallery_street', 'query_satellite', 'query_bev', 'query_drone', 'query_street']}
...
if __name__ == "__main__":
    ...
    evaluate_root(query_name,query_path,gallery_name,gallery_path,os.path.join(absolute_path,'{}'.format(model_dir_name),'{}'.format(opt.name)))
